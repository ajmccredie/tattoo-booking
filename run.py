...
def obtain_calendar():
    ...
    try:
        ...
        for artist in artists:
            events_result = (
                bookings_calendar.events()
                .list(calendarId="primary", timeMin=now, singleEvents=True)
                .execute()
            )
            ...
            for event in events:
                ...
                if artist in event_summary:
                    if "T" in event_start:
                        event_start_details = datetime.datetime.strptime(
                            event_start, "%Y-%m-%dT%H:%M:%SZ"
                        ).date()
                    else:
                        event_start_details = datetime.datetime.strptime(
                            event_start, "%Y-%m-%d"
                        ).date()
                    artist_events.append(
                        (event_start_details, event_summary, event_description)
                    )
        ...
        search_calendar = input(
            "Do you wish to search the calendar for a particular booking? y/n\n"
        )
...
def calendar_search():
    ...
    try:
        ...
        print(
            "Future events found. Please choose how you wish to search the calendar: "
        )
        ...
        if choice.lower() == "a":
            ...
            for name_match in name_matches:
                start = name_match["start"].get(
                    "dateTime", name_match["start"].get("date")
                )
                print(
                    start,
                    ":",
            ...
            for artist_match in artist_matches:
                start = artist_match["start"].get(
                    "dateTime", artist_match["start"].get("date")
                )
                print(
                    start,
                    ":",
                    artist_match["summary"],
                    ".",
                    artist_match["description"],
                    ".",
                )
            print("Returning to bookings menu...\n")
            choose_action()
            ...
            for date_match in date_matches:
                start = date_match["start"].get(
                    "dateTime", date_match["start"].get("date")
                )
                print(
                    start,
                    ":",
...
def search_by_date(events):
    ...
    search_date_input = input(
        "Please enter the date you wish to search(YYYY-MM-DD): \n"
    )
...
def ask_artist_preference_for_deletion():
    ...
    while True:
        ...
        if artist_input in ["1", "2"]:
            ...
            print(
                "Invalid input. Please enter 1 or 2 to choose between (1)Kev and (2)Bev."
            )
...
def phone_valid(client_phone):
    ...
    while True:
        ...
        if phone_entry.isdigit() and len(phone_entry) == 10:
            ...
            print(
                "Invalid phone number. Please check and enter a 10-digit phone number:\n"
            )
...
def convert_date_time_info(length, date_input, time_input):
    ...
    if length == "full":
        ...
        print(
            "Sorry, duration invalid, please enter a valid length of 'full' or 'half'"
        )
        choose_action()
        return
...
def calendar_check(date_request, artist):
    ...
    try:
        ...
        if requested_date > five_months_time:
            print(
                "Error: The date requested is beyond the booking window of 5 months.\nReturning you to the main menu..."
            )
            choose_action()
            return
        elif requested_date < datetime.datetime.now().date():
            print(
                "Error: The date requested is in the past.\nReturning you to the main menu..."
            )
            choose_action()
            return
        ...
        if artist == "No preference":
            print(
                "You selected no preference, so checking the requested date for free artists."
            )
...
def add_to_calendar(client_details):
    ...
    try:
        ...
        start_time = datetime.datetime.strptime(
            client_details["start"], "%Y-%m-%d %H:%M"
        ).isoformat()
        end_time = datetime.datetime.strptime(
            client_details["end"], "%Y-%m-%d %H:%M"
        ).isoformat()
...
def place_booking():
    ...
    date_available, kev_next_date, bev_next_date, assigned_artist = calendar_check(
        date_input, artist
    )
    ...
    if date_available is False:
        print(
            "This date is unavailable, please select another date/artist (next available shown): \n"
        )
    else:
        print(
            "The date you selected is free and will be used in the booking.\n-------\nFor information, the next available dates are also shown:"
        )
    ...
    while date_available is False:
        print(
            "You will need to select another date for that artist to continue your booking (press 'n' to be directed to new date input)\n"
        )
    ...
    while True:
        client_name = input(
            "Please enter client name (no spaces or non-alphabetical characters please): \n"
        )
        if client_name.isalpha():
            break
        else:
            print(
                "Invalid input. Please enter a client name with alphabetical characters only."
            )
        ...
        if age_check == "y" or "n":
            if age_check == "n":
                continue_anyway = input(
                    "Booking will not be able to go ahead, do you wish to continue anyway? y/n\n"
                )
                while True:
    ...
    if age_appropriate is False:
        print(
            "Unable to place booking, clients must be aged 18 or older. Returning to main booking programme..."
        )
        choose_action()
        return
    else:
            ...
            while True:
                ...
                if book_another == "y" or "no":
                    if book_another.lower() == "y":
                        ...
                        logout_confirm = input(
                            "Are you sure you wish to logout of the system? y/n\n"
                        )
                        if logout_confirm == "y":
                            exit()
                        else:
...
def waiting_list_view(events, matched_events):
    ...
    """
    print(
        f"The booking which is being removed is {matched_events[0]['summary'], matched_events[0]['description']}"
    )
    removed_date = datetime.datetime.strptime(
        matched_events[0]["start"]["dateTime"], "%Y-%m-%dT%H:%M:%S%z"
    ).date()
    ...
    for event in events:
        ...
        if (
            waiting_boolean == "True"
            and event_date > removed_date
            and removed_artist == booked_artist
        ):
    ...
    for client in waiting_list_clients[0:5]:
        print(
            f"{index}. Name: {client['name']} Phone: {client['phone']} Tattoo length: {client['tattoo_length']} Date: {client['start']}"
        )
        index += 1
    select_client_for_replacement = input(
        "Select a client from the waiting list to take the cancelled slot by entering the index number (1-5).\n(Select any other key to return to the main menu)\n"
    )
    try:
...
def cancel_booking():
    ...
    try:
        ...
        for event in events:
            ...
            if (
                summary in event_summary
                and date_search == start_time
                and client_name == event_client_name
            ):
                matched_events.append(event)
                print(
                    f"The following booking(s) matching your description has been found: \n",
                    start_time,
                    ": ",
                    event["summary"],
                    ".",
                ...
                if confirm_action.lower() == "n":
                    ...
                    for event in matched_events:
                        ...
                        bookings_calendar.events().delete(
                            calendarId="primary", eventId=event_identifier
                        ).execute()
                        print("Booking deleted.")
                        # question user here as to whether they want to see the next name on the waiting list?
                        while True:
                            waiting_list_request = input(
                                "Do you wish to see the next few clients on the waiting list? y/n\n"
                            )
                            if waiting_list_request == "n":
                                ...
                                selected_client, event_id = waiting_list_view(
                                    events, matched_events
                                )
                                print("Waiting list client has been moved")
                                print(event_id)
                                print(selected_client)
                                tattoo_length_string = selected_client.get(
                                    "tattoo_length", ""
                                )
                                ...
                                time_input = 11
                                (
                                    selected_client["start"],
                                    selected_client["end"],
                                ) = convert_date_time_info(
                                    length, date_request, time_input
                                )
                                ...
                                print(updated_event)
                                print(
                                    f"Here are the new details for that booking: {updated_event['start']}: {updated_event['summary']}, {updated_event['description']}"
                                )
            ...
            print(
                "No matching bookings were found, please check your search criteria.\nReturning to main menu..."
            )
            choose_action()
    except HttpError as error:
        print("An error occurred: %s" % error)
